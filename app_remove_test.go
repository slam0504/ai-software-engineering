...
//
// 只 mustCreate（不 mustStartClaude）：這裡驗的是 token 序列化本身，不是
// teardown 正確性（後者已由 TestRemoveOrderIsFrozen／
// TestRemoveReleasesSlotOnlyAfterAllStepsSucceed 用單一 started session 覆蓋）。
// 三個 dormant slot 已足夠佔滿名額；若改用 mustStartClaude 逐一啟動，
// legacyWSIDFor 第 1 順位（「恰有一個 live host」）會在第二個 session 啟動前就
// 命中第一個 host，讓 StartSession 誤把第二輪送去已 active 的 session 上
//（ErrSessionActive），與本測試無關的既有限制，不必在此觸發。
