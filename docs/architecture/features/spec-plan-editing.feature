Feature: Spec／Plan 手動編輯閉環（A1a）
  ...
  # 契約修訂（A1b-1，2026-09-14）：A1b 在所有寫入入口前加入「寫入前預檢」後，
  # 原本單一的「儲存失敗——digest 衝突」情境分裂成兩種**時序不同**的情況。
  # 原情境的 Given（按下儲存前磁碟已被改動）現在必然先被預檢攔截，**到不了 sentinel**，
  # 故依 owner 2026-09-14 裁定拆為下列兩條。
  # A1a 當時的驗收紀錄不因此變更——A1a 關票時預檢尚不存在，原情境在當時為真。
  # 共同保護（兩條皆須成立）：編輯器內容與受控 buffer 不被覆蓋、已儲存快照與持有的
  # digest 都不被更新、系統不會自動重新載入該檔。

  Scenario: 儲存前已被外部改動——預檢攔截，不送出寫入
    Given 我使編輯器文件改變
    And 該檔在磁碟上已被其他來源改動，我持有的 digest 已過期
    When 我按下儲存
    Then 寫入不會被送出後端
    And 系統明確告知本次儲存已中止，原因是該檔被外部改動
    And 編輯器內容與受控 buffer 不被覆蓋，未儲存狀態依內容比較決定
    And 持有的 digest 與已儲存快照都不被更新
    And 系統不會自動重新載入該檔

  Scenario: 預檢通過後才被外部改動——由後端樂觀鎖拒絕
    Given 我使編輯器文件改變
    And 我按下儲存時磁碟內容仍與我持有的 digest 相符
    When 預檢通過之後、實際寫入之前，該檔在磁碟上被其他來源改動
    Then 寫入被送出後端，並由樂觀鎖拒絕
    And 拒絕依 sentinel 文字契約辨識為 digest 衝突，而不是泛用錯誤
    And 編輯器內容與受控 buffer 不被覆蓋，未儲存狀態依內容比較決定
    And 持有的 digest 與已儲存快照都不被更新
    And 系統不會自動重新載入該檔
