...
- **範圍**：(1) **layer 2b fixture-only differential**——探針其他一切不變，只把 fixture 換成真正的 `testdata/fake-claude.sh`（`FAKE_ORPHAN=1`）；JSON stdin 另列獨立 control；(2) **layer 3 full-path**——owner 明示核准暫時性、build-tag 隔離的 `internal/claude/orphan_diag_test.go`，完整走 `claude.Start → drain → Wait`，分開記錄 `Events()` 關閉與 `Wait()` 返回，並做與既有 oracle 相同的 `kill(-pgid, 0)` 立即／+1／+10／+100ms 取樣與 `ps` `stat` 快照；(3) 真實 fixture 無 token／PID，無可靠身分者只觀察並記 `skipped-no-target`、不送任何訊號；`kill(-pgid,0)` 只證群組存在，zombie 須另有同時點 `ps` `Z` state。**不修改 production、既有測試或 timeout；診斷檔不合併、結案刪除。B2a 維持 blocked。**
- **驗收條件**：(1) layer 2b／2b-json／3 各 ×100 於 macOS＋ubuntu 兩 replica 的 artifact 齊全、`invalidEvidence` 0；(2) 對照表判讀寫明差異落在 fixture、stdin 或 `claude.Session` 路徑；(3) `kill(-pgid,0)` 結果未被寫成 zombie 除非同時點 `ps` 有 `Z`；(4) register #7 依結論更新，需修改則另開 implementation 票。
...
| B2a | 最小 CI workflows（frontend／go／wails-build／checksums）＋SHA256SUMS 收斂＋README 可重現順序＋B3b artifact（rev16 拆自 B2） | **0.6**（5.0–7.0 hr） | **Gate A 停止，待 B2c**（PR #1 開啟未合併；plan `docs/superpowers/plans/2026-09-05-b2-minimal-ci-ruleset.md`）。runner 等候不計工時 |
| B2b | main ruleset＋enforcement 實證 (5a)–(5e)＋`ci-merge-policy.md`＋(6) CI 冷啟動量測 n=5（rev16 拆自 B2） | **0.7**（6.0–8.5 hr） | 未開始；依賴 B2a 合併；GitHub 設定面變更逐步授權；register 回填版本 v4 |
...
| B2c-2 | round 2：fixture-only differential＋full-path（`internal/claude` 暫時性診斷檔，owner 核准）（rev17 新增，限診斷） | **0.3**（3 hr） | 未開始；plan rev1 待短複審；沿用 `b2c/diag`，一次推送；不送訊號給無身分程序 |
