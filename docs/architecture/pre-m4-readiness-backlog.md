...
- **依賴／裁決**：依賴 B2a 的 workflow 檔（診斷 job 可掛 non-required、`workflow_dispatch`）；owner 2026-09-05 裁定開票、初估 **0.4 pt**（0.3–0.5），plan gate 再核對。
...
| B2c | `TestOrphanDoesNotHangNormalExit` CI-only 逾時診斷（rev16 新增，限診斷） | **0.4**（0.3–0.5） | 未開始；owner 初估並於 plan gate 維持 0.4；不修改既有 production／測試檔、不得放寬 timeout 或加 retry；允許 `b2c/diag` 上暫時性白箱診斷測試檔（不合併、結案刪除） |
...
**小計（rev16 更新——B2 1.2→B2a 0.6＋B2b 0.7＋B2c 0.4＝1.7）**：A 軌 3.0 pt（30 hr）｜B 軌 **12.21 pt（122.05 hr）**｜C 軌 3.5 pt（35 hr）｜**合計 18.71 pt（約 187.05 hr 實際工作）**。排程換算（團隊 throughput 約 0.6 pt／day）為另一維度，於排程時另算，不與工程量混用。
