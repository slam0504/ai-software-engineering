# Pre-M4 Readiness Backlog（rev20·估點版）

> 版本：rev20（2026-09-07，**B2c-4 關票（決策票，owner APPROVED；裁定記錄 `docs/superpowers/plans/2026-09-07-b2c-4-supervisor-cleanup-contract-decision.md` rev3）：supervisor cleanup 契約改為「有界清理；無法確認時強制解除本端 pipe 等待並揭露未完成」，不再宣稱單次或有限次 KILL 絕對保證群組終止；新增 implementation 票 B2c-5（proc 有界清理狀態機＋`CleanupIncomplete` 揭露，0.6 pt）、B2c-6（測試 oracle 有界化，只以 ESRCH 為消失，0.3 pt）、B2c-7（`b2c7/verify` CI 驗證與 register #7 → resolved 完整條件，0.2 pt），合計 11.0 hr → 1.1 pt；register #7 維持 unresolved（v6，EPERM 勘誤）；B2a 維持 blocked，待 B2c-7 使 #7 resolved（register v7）後解除，rebase 後仍須重做 Gate A；B2 合計 3.8 pt、B 軌 143.05 hr → 14.31 pt、合計 208.05 hr → 20.81 pt**）
> 前版：rev19（2026-09-07，**B2c-3 關票（結論：macOS 機制解讀定位為 fork 窗口——KILL 送達時正處於建立中的成員被 XNU `killpg1` 靜默略過，CI production 順序 36–54% 重現、群組約 30 s 後消失已直接觀察、經身分驗證的第二次群組 KILL 於探針條件下 109／109 清除；送達瞬間未直接觀察、責任邊界待 B2c-4）；估點依 owner design gate 改為 0.5 pt（5.0 hr）；register #7 維持未解決（v6，補機制欄）；B2c-4 依賴成立；B2a 維持 blocked，待 B2c-4 與 #7 最終處置；B2 合計 2.7 pt、B 軌 132.05 hr → 13.21 pt、合計 197.05 hr → 19.71 pt**）
...
| B2c-5 | proc supervisor 有界清理（狀態機、fail-loud、`CleanupIncomplete` 傳遞、seam 與白箱六案；rev20 新增） | **0.6**（5.0–7.0 hr） | 未開始；依賴 B2c-4；owner 2026-09-07 採用估點 |
| B2c-6 | 測試 oracle 有界化（只以 ESRCH 為消失；四套件；rev20 新增） | **0.3**（2.5–3.5 hr） | 未開始；順序在 B2c-5 後；owner 2026-09-07 採用估點 |
