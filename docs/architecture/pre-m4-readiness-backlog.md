...
- **rev16 拆票與現況（owner 2026-09-05 於 B2 plan gate 裁定）**：B2 拆為 **B2a**（workflow 四 job、checksum 收斂、README 可重現順序、B3b artifact；0.6 pt）、**B2b**（ruleset、enforcement 實證 (5a)–(5e)、`ci-merge-policy.md`、(6) 量測；0.7 pt）、**B2c**（下條，診斷；0.4 pt）。**B2a 狀態：Gate A 停止，待 B2c-3／B2c-4 與 #7 最終處置**（B2c／B2c-2 已關票）——PR #1（`b2a/ci-workflows`，head `ffcd161`）首次 run `33952217785` 四 job 全綠、failure-path control run `33952896114` 實證成立，但正式綠燈候選 run `33953144191` attempt 1／2 於 `internal/claude/TestOrphanDoesNotHangNormalExit` 連續紅燈（非 register 八條、本機不可重現、根因未確認），依 owner 規則停止；**禁止合併、禁止 main-run 步驟、不再重跑**。register v3 已登記為 Go 表 #7 候選。B2a 其他事實：`docs/architecture/SHA256SUMS` 過期（治理文件 v2.2–v2.4 後未重算）已於 PR 內收斂為 m0／m1／m1.5 三份（D3）；CI 釘 `macos-15-intel`＋Go 1.26.5（D2，A4 落地後改讀 go.mod）。
...
- **依賴／裁決**：依賴 B2c-2 結案（已成立）。**估點 0.4 pt（3.5–4.5 hr，中位 4.0）——owner 2026-09-06 採用，計入小計。**
...
- **依賴／裁決**：依賴 B2c-3 產出（可先以現有事實預裁定並標假設）。**估點 0.2 pt（1.5–2.5 hr，中位 2.0）——owner 2026-09-06 採用，計入小計。**
...
| B2a | 最小 CI workflows（frontend／go／wails-build／checksums）＋SHA256SUMS 收斂＋README 可重現順序＋B3b artifact（rev16 拆自 B2） | **0.6**（5.0–7.0 hr） | **Gate A 停止，待 B2c-3／B2c-4 與 #7 最終處置**（PR #1 開啟未合併；plan `docs/superpowers/plans/2026-09-05-b2-minimal-ci-ruleset.md`）。runner 等候不計工時 |
...
| B2c-3 | macOS 群組 KILL 後成員存活窗口定位（rev18 新增，限診斷／spike） | **0.4**（3.5–4.5 hr） | 未開始；owner 2026-09-06 採用估點 |
| B2c-4 | supervisor cleanup 契約裁定與 #7 處置（rev18 新增，決策票，不改 code） | **0.2**（1.5–2.5 hr） | 未開始；依賴 B2c-3；owner 2026-09-06 採用估點 |
...
**小計（rev18 更新——B2 合計 B2a 0.6＋B2b 0.7＋B2c 0.4＋B2c-2 0.3＋B2c-3 0.4＋B2c-4 0.2＝2.6）**：A 軌 3.0 pt（30 hr）｜B 軌 **13.11 pt（131.05 hr）**｜C 軌 3.5 pt（35 hr）｜**合計 19.61 pt（約 196.05 hr 實際工作）**。排程換算（團隊 throughput 約 0.6 pt／day）為另一維度，於排程時另算，不與工程量混用。
...
**單位與捨入規則（rev8 新增，避免兩組答案）**：**hr 為權威單位，pt 一律由 hr ÷ 10 衍生**。估點表各票的 pt 為顯示用值、四捨五入至小數點後兩位；**合計一律以未捨入的 hr 總和換算，不得由各票顯示值相加**。B1a 四票中位 hr 為 11.25／10.10／**7.70**／9.50，**總和 38.55 hr → 3.86 pt**（rev12 更新；rev11 為 35.60 hr → 3.56 pt，rev8 為 37.40 hr → 3.74 pt）；本版四個顯示值（1.13＋1.01＋0.77＋0.95）相加**恰好也是 3.86 pt**，與未捨入換算一致（rev11 曾有 0.01 pt 的捨入差額，本版無），但**合計仍一律以未捨入 hr 總和換算**，此處相符只是巧合、不改規則。B 軌與合計均以 **hr** 推導，不使用任何已捨入的 pt 值：B 軌 `(92.5 − 15 ＋ 38.55 ＋ 1.0 ＋ 5.0 ＋ 3.0 ＋ 4.0 ＋ 2.0) hr = 131.05 hr → 13.105 pt → **13.11 pt**`（rev15：B1b ＋1.0；rev16：B2 ＋5.0；rev17：B2c-2 ＋3.0；rev18：B2c-3 ＋4.0、B2c-4 ＋2.0）；全部合計 `(157.5 − 15 ＋ 38.55 ＋ 1.0 ＋ 5.0 ＋ 3.0 ＋ 4.0 ＋ 2.0) hr = 196.05 hr → 19.605 pt → **19.61 pt**`（小數點後第三位 5 一律進位；rev17 為 125.05／190.05 hr）。逐條原始數字見附錄 C。
