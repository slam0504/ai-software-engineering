...
**變異目標**：mutation 應植入**本票新增或修改、且直接承載本票契約**的 production code。不得為製造紅燈而修改本票未更動的共用基礎設施；可由呼叫端等本票局部變異表達的錯誤，**不得下沉至共用 helper**。變異若造成非預期 panic 或跨範圍連帶失敗，**不構成「紅在正題」**。
...
**生產碼註解**引用**同檔案內**的宣告、函式或欄位時，**不得嵌入行號**；應使用符號名或程式結構定位。**文件中的行號只能作為附帶的當下定位資訊，不得取代符號錨點。**

理由：同檔案內嵌的行號會被**改動本身**作廢——當新增內容落在被引用符號的上方，該行號在落地的那一刻就失準。這不是隨時間緩慢漂移，而是同一次改動即刻造成的失準，且審查時容易被當成「寫下時是對的」而放過。

實務形狀（皆為實際發生過的案例）：
- 測試檔註解引用同檔案某段既有斷言的行號，該 commit 自己在其上方新增內容後失準。
- 文件的修訂記錄自我引用同文件某段落的行號，該 commit 自己在標頭插入一行後整體位移。
- 實作範本的 doc comment 內嵌同檔案某 helper 的行號，範本自己新增的欄位與常數落在該 helper 上方，落地即失準。

適用邊界：plan／spec／設計文件的散文段落**可以**保留「符號名＋（現 line N）」形式——符號名是錨點、行號是輔助；但單獨的行號不得作為唯一定位依據。
...
起因：短期內第三次同型缺陷——B6 Task 7 的測試範本註解引用 `fsync_test.go:229`（實際斷言在 243-245）；B5 spec 的 rev10 修訂記錄自我引用「§3.6 段 line 133」，而同一 commit 在版本標頭插入一行使該段落移至 134；B6 Task 8 的 `FreezeTurns` doc comment 內嵌「committedSlotLocked（現 manager.go:375）」，而該票自己新增的哨兵區塊與 slot 欄位都在其上方，落地後該函式移至 387 行。

共同形狀不是「隨時間緩慢漂移」，而是**同一次改動即刻造成的失準**——行號在寫下時正確，落地那一刻就錯，且審查時容易被當成「寫下時是對的」而放過。第三例更直接導致 implementation commit 作廢重做。

§6.8 新增：生產碼註解引用同檔案內的宣告／函式／欄位時不得嵌入行號，應使用符號名或程式結構定位；文件中的行號只能作為附帶的當下定位資訊、不得取代符號錨點。plan／spec 散文段的「符號名＋（現 line N）」形式仍合規。

本次為文件治理修正，**不改變任何既有 gate 設計、角色權限或階段順序**，亦不追溯改寫任何已結案票的驗收裁定。

### v2.3（2026-09-02）— §6.7 變異目標規則（doc-only 治理修正）

起因：B6 Task 8 的 design gate 實跑時發現，mutation 表某一列把變異目標指向 `appcore.Manager.committedSlotLocked`——那是全檔 19 個呼叫點共用的**既有**基礎設施，不是本票新增的碼。實測兩種落地皆有不可控連帶：一種在未知 WSID 時對 nil slot 取欄位而 nil deref panic、整個測試二進位崩潰；另一種雖讓目標 subtest 紅在自身斷言，卻同時波及兩個與本票無關的既有測試。該列因此無法達成其餘各列的「全包只紅目標項」模式。

這是已記錄的三型鑑別力失效形狀（guard 遮蔽／斷言太弱／測資未隔離目標欄位）之外的**第四型：變異目標選錯**。修正後改為本票本地變異（在新寫的函式內吞掉共用 helper 回傳的錯誤），無 panic、無跨範圍連帶。

§6.7 新增「變異目標」段落：mutation 須植入本票新增或修改、且直接承載本票契約的 production code；不得為製造紅燈而修改本票未更動的共用基礎設施；可由呼叫端等本票局部變異表達的錯誤不得下沉至共用 helper；變異若造成非預期 panic 或跨範圍連帶失敗，不構成「紅在正題」。
