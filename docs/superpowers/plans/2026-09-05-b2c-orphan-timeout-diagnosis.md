...
- **證據完整性**：Start／stdin write／scanner error／每個 `ps` 寫檔／partial／JSONL encode／close／summary 任一失敗都累積 `invalidEvidence`，保留可用 artifact 後以 `t.Fatalf` 非零結束（layer 2 rc≠0 ＝ 證據不完整，不是重現）。
...
## 尚未完成
- Task 1 與 Task 2 Step 1 完成（本機，含第二輪 P1 修正與強制控制）；Task 2 Step 2 待 owner 授權推送；Task 2 Step 3、Task 3 未執行。plan rev5／rev6 已 commit（`a48018c`／`6ae5624`）。
