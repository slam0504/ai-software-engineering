...
## 尚未完成
- Task 1、Task 2 完成；Task 3 結論已依 owner 裁定收斂，round 2 拆為 B2c-2（plan 另立）。剩：register #7 回寫（D6 措辭）＋backlog rev17（docs-only 落 main，待 push 授權）。plan rev5–rev10 已 commit（rev9 `cbd7d3b`、rev10 `d5b0967`），rev11 待 commit。register v4／backlog rev17 已推送 main（`5fc3373`）。**B2c 本票剩餘事項：無；關票。**`b2c/diag` 遠端 head `d12b13b`；本機領先的 rev9／rev10 **不單獨推送**（會無效觸發四 job），待 B2c-2 檔案備齊後一次推送。
