...
- Produces（preflight 校正——原版 Step 3 用裸 `m.slots[w]` 繞過 `committedSlotLocked`、且 Step 3 錯誤處理用 `fmt.Errorf` 臨時字串與本段原「皆回具名 error」的承諾自相矛盾，owner 裁決二擇一：改採具名哨兵，二者對齊）：
  - 新哨兵 `ErrTurnsFrozen = errors.New("appcore: turns frozen for this workspace session")`，沿本檔既有 **16** 個 `var ErrXxx = errors.New(...)` 慣例（manager.go:13-39）追加。
...
`var (...)` 錯誤哨兵區塊追加 `ErrTurnsFrozen`（沿既有 16 個哨兵慣例）：
...
| # | Mutation（植入處／情境） | 預期紅燈斷言 | 對應測試 |
|---|---|---|---|
| 1 | `beginSubmitLocked` 的 `sl.turnsFrozen` guard 被移除 | frozen slot 的 `BeginSubmit` 應回 `ErrTurnsFrozen`（改寫入成功） | `TestFreezeTurnsBlocksBeginSubmit`。**預期連帶**：`TestFreezeTurnsMonotonicAndUnknownWSID/monotonic_idempotent_and_sticky` 亦會轉紅——其收尾斷言同樣呼叫 `BeginSubmit` 並期待 `ErrTurnsFrozen`，與本列移除的是同一段 guard，屬「同一 guard、兩個獨立呼叫點」的結構性必然，非新增鑑別力、也非遮蔽 |
...
| 3 | **`FreezeTurns` 內**把 `committedSlotLocked` 的回錯分支改成吞掉錯誤（`if err != nil { return nil }`），誤放行未知 WSID。**變異必須落在本票新增的 `FreezeTurns` 內，不得改共用的 `committedSlotLocked` 本體**——後者是全檔 19 個呼叫點共用的既有基礎設施，變異它會 nil deref panic 並波及 `TestRemoveSessionReleasesIdleSlot`／`TestUnknownWSIDNeverCreatesSlot` 等與本票無關的既有測試，不構成「紅在正題」（見治理文件 §6.7 變異目標規則） | 完全未知的 WSID 應回 `ErrSessionNotFound`（改回 `nil`） | `TestFreezeTurnsMonotonicAndUnknownWSID/unknown_wsid_rejected`。**預期連帶**：本變異同時讓 `uncommitted_reservation_rejected` 轉紅——兩條路徑共用 `FreezeTurns` 內同一句 error 傳遞，屬結構性必然。鑑別力歸屬仍唯一：`unknown_wsid_rejected` **只有本列**能讓它紅（#4 的裸 map 對未知 WSID 仍會回錯） |
...
1. **套用**：證明 mutation diff 已實際落到 production 檔（`git diff`／`diff` 顯示變更存在）且檔案內容 hash 已改變（變異前後 `shasum`／`sha256sum` 不同）。
2. **轉紅**：表中指定的測試（含 subtest 名）確實 FAIL，且**紅在正題**——失敗訊息必須是該測試自己的斷言訊息，不是撞到其他 guard、或由其他測試連帶失敗。#7／#9 兩項鎖洩漏 mutation 須用**完整 regex** `-run 'Freeze|Frozen|ResolveApproval'` 驗證，且該次指令內全部相關測試都要在外層 `-timeout` 之內、以各自的斷言訊息轉紅或通過——不得有任何測試卡到外層 timeout。
3. **還原**：`cp` 備份還原（不得用 `git checkout --`），還原後檔案 hash 與變異前一致（byte-identical），證明無殘留。
4. **轉綠（owner 2026-09-02 二次裁定，調整自本 Step 5a 初版的「每項全包」定義）**：以與第 2 步相同的 focused `-race` 指令（`go test -race -run 'Freeze|Frozen|ResolveApproval' -count=1 -timeout 300s .`）確認轉綠，**不要求每項各自跑一次全包**——理由：每個 mutation 在獨立的 `go test` 程序中執行，還原後的 focused 綠燈已足以驗證正題（該 mutation 影響的行為已還原），全包回歸留給下面的整合步驟一次做完，避免 9 項各自等待 root 包全跑（約 200-400 秒／次）的重複時間成本。9 項全部完成（含還原＋focused 轉綠）後，**統一執行一次 `go test -race -count=1 .`**，確認 root 包整合回歸全綠；這次全包才是本票的整合驗收基準，focused 轉綠是各項 mutation 自身的正題證據。
...
| 9 | `resolveApproval` 新增的 frozen-allow 分支（`if allow && a.apprFrozen[p.wsid] { ... }`）漏 `a.apprMu.Unlock()`（鎖洩漏） | frozen allow 被拒絕後，同一筆 pending 的後續 deny 不得卡死、且應成功 | `TestFrozenAllowLeavesLockAndPendingUsable`；`TestFreezeImplementationSessionDrainsAndBlocksAllow/late_pending_allow_blocked_deny_legal` 命中同一個早退分支缺陷會**結構性連帶紅**——該 subtest 使用獨立 App／WSID／freeze setup，deny 呼叫經 `assertNoApprovalDeadlock` 於 2 秒內轉紅，**不得卡到外層 `-timeout`** |
