...
- [x] Gate A1a-1（2026-09-07）與 Gate A1a-2（2026-09-08，綁定 `85d6418`）皆通過，兩票證據分別齊備 → **A1a aggregate 本機驗收完成**。
- [x] **正式關票**（owner 2026-09-08 裁定，綁定 main `f9e2524e6ce362f1be42de9b56ce8b724a1b160d`）：PR #4 rebase merge 進 main、main push run `34218094834` 四個 required job 全綠。詳見第十四節。
...
**遺失範圍**：`/tmp/a1a-1/`（120 檔）與 `/tmp/a1a-2/`（143 檔）**整個目錄消失**。內容依**既有清單**（§11.4b、§12.5）為：A1a-1——D5 spike、build／wails build／app 啟動輸出、隔離模式重現紀錄、`table.json`、`results.json`、38 組紅綠日誌、harness；A1a-2——harness v1／v2、變異表與合併索引、24 組紅綠日誌與 **24 份**原始位元組備份、`vitest-final.txt`、build 與 `go test` 輸出、29 張 GUI 截圖、執行檔 hash 與 `models.ts` 再生 diff。**備份只有 A1a-2 的 24 份**——A1a-1 當時使用 harness v1（`git checkout` 還原），本就沒有備份目錄；先前寫「24＋24 份」無既有清單支持，已更正。已確認 `/private/tmp`、`~/Library/Caches` 與 repo 內皆無副本。**清除時點與原因未知**——目錄時間戳不能證明清除時間或成因。
...
**現況**：**原封存目前無法調閱；現有回收片段不足以完整複查。** GUI 截圖與當時的執行環境無法重建。

**回收狀態（部分回收，非原封存恢復）**：`~/.local/share/sdlc-evidence/` 下 **`recovered-from-session/` 合計 5 檔**（A1a-1 2 檔、A1a-2 3 檔），另有證據區根目錄 `README.md` 1 檔，**合計 6 檔**——隔離模式重現量測（僅指令與計數，**無失敗原文**）、mutation 目標斷言表（斷言為節錄）、GUI 逐案結果（截圖無法重建）、各自的 `PROVENANCE.md`。

**來源定位**：全部擷取自本會話轉錄，session ID `df399e66-649a-4a05-b0aa-506259b0487f`；各片段的產生時點（依會話時間標記，**為約略時間、非檔案 metadata**）——隔離模式量測約 2026-09-07 16:26–16:32；A1a-2 mutation 目標斷言表約 2026-09-08 11:0x（更正計數後的版本）；GUI 逐案結果約 2026-09-08 10:15–10:23。**不得作為原始輸出、不得用於重建 manifest、不得宣稱與原封存等價**。

**往後的保存規約（owner 2026-09-08 裁定）**：證據改存 `~/.local/share/sdlc-evidence/<ticket>/<受測SHA>/<批次>/` 並納入備份；`/tmp` **僅作暫存**；**複製到持久位置並重新驗證 manifest 之後**，才可在文件回填「封存完成」。**目前只能確認保存位置已建立**——本次未取得該路徑已納入備份的證據。


## 十四、關票紀錄（rev15 新增；owner 2026-09-08 裁定）

**綁定**：main `f9e2524e6ce362f1be42de9b56ce8b724a1b160d`。

| 項目 | 事實 |
|---|---|
| PR | **#4**「A1a：同步 Spec／Plan 編輯內容並保護未儲存導覽」，base `main`、head `a1a/editor-sync`＝`a9b8384ed202b6584da589d86ba53739a8ad1ff5` |
| 合併 | **MERGED**（2026-09-08T10:56:46Z），rebase merge，merge commit＝`f9e2524e6ce362f1be42de9b56ce8b724a1b160d`（＝目前 `origin/main`） |
| 合併後 CI | main push run **`34218094834`**（head `f9e2524`、event `push`）——`frontend`／`go`／`wails-build`／`checksums` **四個 required job 全部 success** |
| 期間紅燈 | PR CI run `34202299786` 的 `go` job 在 **gofmt 格式檢查**失敗中止（owner 裁定為**格式檢查缺漏，不是 Go 測試失敗**；build／vet／test 未執行），修正後重新推送 `c549d45` 取得全綠 |
| 分支 | `a1a/editor-sync` 遠端仍在 `a9b8384`，**刪除另案未授權** |

### 14.1 關票時仍成立的三項限制（不因關票消滅）

1. **原封存遺失**：`/tmp/a1a-1`（120 檔）與 `/tmp/a1a-2`（143 檔）已整個消失、清除時點與原因未知，**原封存目前無法調閱，現有回收片段不足以完整複查**（第十三節）。Gate 裁定作成於證據可調閱時、owner 已逐項核對，事後遺失不改變當時的判定，但**也不代表現在能重新查核**。
2. **GUI 涵蓋限制**：A1a-2 的 Wails GUI 證據由 **agent 透過 osascript 取得**（非人工操作、非 owner 實機確認；A1a-1 的人工驗收才是 owner 實機確認）；**寫入進行中**與**重新送核導向**兩項情境 **GUI 未涵蓋**，由 G6／H1 系列與 G4／G5／H4-S／H5-P 的自動化證據承接；**H5-P 只有自動化證據**。
3. **持久證據區未確認納入備份**：`~/.local/share/sdlc-evidence/` **只能確認保存位置已建立**，本次未取得該路徑已納入備份的證據（Time Machine 無指定目的地）。備份目的地指定為**另案**。

### 14.2 四類證據來源（**分開標示，不得互相回補或混稱**）

**計數口徑（全表一致）**：檔數一律指**資料檔**，各批次自己的 `EVIDENCE.sha256` 另計、不含在內；(a) 的 120／143 為當時紀錄的 manifest 涵蓋檔數，(b) 該區無 manifest。

| 類別 | 內容 | 現況 |
|---|---|---|
| (a) **A1a 原封存** | `/tmp/a1a-1` 120 檔、`/tmp/a1a-2` 143 檔（紅綠日誌、備份、GUI 截圖、原始輸出） | **已遺失，一檔都沒有回收**。當時的 `shasum -c` 為 120/120、143/143 OK，manifest hash 見第十三節——該完整性由**當時的驗證紀錄**支持，hash 本身不能證明檔案曾存在或完整 |
| (b) **A1a 會話回收片段** | `a1a-1/2bd4890/recovered-from-session/` **2 份**、`a1a-2/f719d43/recovered-from-session/` **3 份**，加證據區根目錄 `README.md` 共 **6 份**（此區無 manifest） | 擷取自會話轉錄（session `df399e66-649a-4a05-b0aa-506259b0487f`），**節錄且有截斷**、時點為約略值；**不是原始輸出、不得用於重建 manifest、不得宣稱與 (a) 等價** |
| (c) **B2b-2 的 CI 資料** | `b2b-2/review-20260908T175707/`：**11 份資料檔**（五筆 run 的 jobs JSON 與日誌、`index.json`）＋1 份 manifest；複查 **11/11 OK** | **2026-09-08 自 GitHub 重新取得**。這是 **B2b-2 的 CI 量測資料**，與 A1a 的封存無關；「部分資料已重新取得」只適用於此類，**不適用於 (a)** |
| (d) **A1a 交付期間新建的證據** | `a1a/<受測SHA>/` 各批次，內容為 gate 的逐項期望輸出與 stderr、push／PR／合併的實際輸出、CI 輪詢與 run／jobs JSON。**計數口徑：資料檔不含各批次自己的 manifest**——`f2a5ebb/pre-push-review` **25 份資料檔**、`f2a5ebb/ci-review-34202299786-attempt1-codex` **17 份**、`c549d45` **19 份**、`a9b8384` **18 份**、`a9b8384/merge` **22 份**，各批次另有 1 份 `EVIDENCE.sha256` | **本次新建**。複查結果：`ci-review-…` 17/17、`c549d45` 19/19、`a9b8384` 18/18、`a9b8384/merge` 22/22 **全數通過、無 FAILED**；**`pre-push-review` 的 manifest 只涵蓋 2 份資料檔**（`pr-draft.md`／`readback.md`，2/2 OK），**其餘 23 份未涵蓋**。這類證據記錄的是**交付動作**（推送、開 PR、CI、合併），**不能回補 (a) 的實作驗證證據** |


## 修訂記錄

- rev15（2026-09-08）：**A1a 正式關票回填**（owner 裁定，綁定 main `f9e2524e6ce362f1be42de9b56ce8b724a1b160d`）——PR #4（head `a9b8384`）rebase merge 進 main、main push run `34218094834` 四個 required job 全綠；期間 PR CI run `34202299786` 的 `go` job 因 **gofmt 格式檢查**中止（owner 裁定非 Go 測試失敗），修正後重新取得全綠。新增第十四節：關票綁定表、**14.1 三項限制**（原封存遺失／GUI 涵蓋限制與操作歸屬／持久證據區只確認位置已建立）、**14.2 四類證據來源分開標示**（(a) 原封存已遺失且一檔未回收、(b) 會話回收 6 檔為節錄、(c) **B2b-2 的 CI 資料才是自 GitHub 重新取得**、(d) 交付期間新建證據不能回補 (a)），並揭露 `pre-push-review` 的 manifest 只涵蓋 25 份資料檔中的 2 份（其餘 23 份未涵蓋）；(d) 的檔數一律以**不含 manifest 的資料檔**計。Gate、測試、mutation 與 GUI 結論不變、本 rev 未重跑任何驗證；分支 `a1a/editor-sync` 刪除另案未授權。
- rev14（2026-09-08）：遺失紀錄的精度修正（owner 複核 `bf1f42a`）——(1) 回收檔數：`recovered-from-session/` **合計 5 檔**（A1a-1 2、A1a-2 3），加證據區根目錄 README 才是 6 檔；(2) 補**來源定位**：session ID `df399e66-649a-4a05-b0aa-506259b0487f` 與各片段的約略產生時點（依會話時間標記，非檔案 metadata）；(3) 備份份數更正——**只有 A1a-2 的 24 份**有既有清單（§12.5）支持，A1a-1 當時用 harness v1 的 `git checkout` 還原、本就沒有備份目錄，先前「24＋24 份」無依據；(4) 「複查只能重跑」改為「**原封存目前無法調閱；現有回收片段不足以完整複查**」；(5) 持久位置**只能確認已建立**，本次未取得其已納入備份的證據。Gate、mutation 與 GUI 結論不變。
