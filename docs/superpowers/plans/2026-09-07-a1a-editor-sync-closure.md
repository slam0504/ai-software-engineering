...
- [x] 獨立複驗：**沒有任何目標失敗是環境造成的**——38 份紅燈日誌都含目標測試的失敗行與其斷言訊息，且無一是因 CM6 未掛載或逾時而中止。**更正（owner 2026-09-07 複核指出）**：這不等於「沒有環境錯誤訊息」——38 份中有 **35 份**仍含 `getClientRects is not a function` 的 jsdom 量測 stderr 雜訊，該雜訊不是失敗原因；`results.json` 的 `msg` 欄位先前常誤取此雜訊而非目標斷言，**已離線修正摘要擷取邏輯（排除雜訊、優先取斷言行），原始日誌保留、mutation 未重跑**
