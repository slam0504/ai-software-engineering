...
- [x] 新增 `docs/architecture/ci-merge-policy.md` v1.1（§1–§9 對應 D4 (i)–(ix)；附錄 A payload＋GET、附錄 B 有效規則＋PATCH 前後、附錄 C 三狀態證據）。**rev6 於同一 PR 內更新為 v1.2**（§9 量測出處改為 B2b-2 的取樣規則），合併進 main 的是 v1.2。
...
- [x] （rev6）PR #3 最後一次 push：plan rev6＋backlog rev27（範圍拆分、B2b-2 立票、B2b 狀態）＝`ea5123a`（第一次 gated push 於 7 項鎖定通過後 SSH 逾時 rc 128，唯讀確認遠端未變＝未寫入，同一命令第二次成功）→ run `34080497639`（pull_request、attempt 1、建立 03:41:29Z 晚於 rev6 核准）四 required 全綠，指紋與樣本 1／2 相同 → **B2b-2 樣本 3/5**。register v8 與 B2b-2 關票由 B2b-2 負責。歷史原文：五樣本固定 → 一次彙整 commit（register v8、backlog rev27、plan 最終回填）。
- [x] （授權，含自動 main run）rebase merge PR #3 →（授權，分三步）刪 `b2b/closure`。執行結果：
  - **合併**（2026-09-07 03:58:23Z）：13 項鎖定（本機乾淨／HEAD、7 commit 無 merge、遠端 closure 與 main、PR OPEN／非 draft／未合併、head 與 base、`MERGEABLE|CLEAN`、ruleset active 無 bypass、有效規則五項、四 required contexts app 15368、`ea5123a` 四 check-runs 全 success、repo rebase-only）先以無寫入模擬全通過，再單次執行 `gh pr merge 3 --rebase --match-head-commit ea5123a…`（rc 0、無輸出，改唯讀查證）。PR #3 `MERGED`、mergeCommit `3ea31ea`；main `1bbb47a`→`3ea31ea`，tree `a6a8fef` 與 `ea5123a` 相同、`git diff` 空；自 base 增加 **7 個線性 commit**（merges 0、`1bbb47a` 為祖先），rebase 前後逐一 tree／subject 一對一相符（`dc28358→02008c7` … `ea5123a→3ea31ea`）。
  - **main push run** `34081432585`（push、attempt 1、建立 03:58:26Z→04:06:13Z，467 s）completed/success：`checksums` 7 s／`frontend` 51 s／`wails-build` 160 s／`go` 408 s，`3ea31ea` 四 check-runs 全 success（app 15368）；該 SHA 僅此一個 run，未 rerun／dispatch。**不計入 B2b-2 樣本**。輪詢中一次 `gh api` i/o timeout（空結果）——owner 裁定唯讀輪詢可間隔重試、失敗當下狀態為「未知」不得當成功，完成後另以獨立查詢複驗（本次已做）。
  - **分支刪除**（D7 (14)，分步授權）：Step 0 本機 `checkout main`＋`merge --ff-only 3ea31ea`；Step 1 原 9 項鎖定通過後 `git push origin --force-with-lease=refs/heads/b2b/closure:ea5123a… --delete b2b/closure`；Step 2 另建**刪後** 8 項鎖定（含「遠端查詢成功且 ref 不存在」以 sentinel 表示、查詢 rc≠0 即失敗）通過後 `git branch -D b2b/closure`。複驗：遠端 heads 僅 `main`＋`m3b-multi-session`、本機無該分支、PR #3 仍 MERGED、ruleset 仍 active、刪除未觸發新 run。舊 SHA（`ea5123a`／`57b2448`／`5a83717`／`417791a`）依 owner 裁定不打 tag，內容以 main 上 tree 相同的 commit 保留。
  - B2b 關票的合併 SHA／main run 已於 backlog **rev28** 補記，未另開自我參照的回填 PR。
...
- [x] `ci-merge-policy.md` **v1.2**（含維護程序 §4–§7）落地並由 automation plan §12 與 README 指向；PR #3 以 rebase merge 落地（含 plan rev6、backlog rev27）→ main `3ea31ea`。
- [x] （rev6 改寫）量測移交完成：樣本 1／2 記錄、採集工具（`d6.sh`／`d6-summarize.sh`／gate）、D5／D6 取樣與指紋規則已寫入本 plan 與 backlog B2b-2；**register v8 與 B2b-2 關票不在本 Gate**。每步外部寫入皆有授權與前後狀態紀錄。歷史原文：D6 五樣本表與 register v8、backlog rev27 落地。

## 修訂記錄

- rev8（2026-09-07）：複核修正，事實與結論不變——(a) 結案敘述（狀態行、Gate B）引用的政策版本 v1.1 → **v1.2**，Task 3 補註「rev6 於同一 PR 內更新為 v1.2」，v1／v1.1 保留為歷史；(b) D7 項次更正：最後一次 push 為 **(12)**，(11) 是量測移交（B2b-2 接手、量測未完成），rev7 的「(11)–(14) 全部完成」措辭改寫為分別敘述；(c) 未展開的速記改為完整敘述。
- rev7（2026-09-07；項次與版本引用由 rev8 更正）：執行回填與關票——**D7 (12)**（最後一次 push）push `ea5123a`（一次 SSH 逾時未寫入、同命令重試成功）與樣本 3 候選 run `34080497639`（合格，計入 B2b-2）；**D7 (11)** 為「量測改由 B2b-2 自正常開發 PR 採樣」，即**移交完成、量測本身未完成**；D7 (13) 13 項鎖定後 rebase merge PR #3 → main `3ea31ea`（tree 相同、七個線性 commit、逐一對應）＋main push run `34081432585` 四 job 全綠；D7 (14) 分三步刪除 `b2b/closure`（Step 2 另建刪後 gate）；Task 4 與 Gate B 全部勾選；狀態行改為已關票；量測與 register v8 移交 B2b-2。設計內容 D1–D8 不變。
