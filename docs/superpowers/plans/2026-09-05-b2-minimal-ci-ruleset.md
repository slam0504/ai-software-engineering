...
- [ ] （本 rev6＋backlog rev25 push 後觸發，另案授權；run id／SHA／耗時於 rev7 回填）**closure-commit main run**：plan／backlog closure commit push 後，final B2a HEAD 再跑四個 job 全綠，與 implementation main run 分開記錄；Gate A 綁 final HEAD。
...
## Gate A（B2a 完成條件）
- [ ] PR 內四個 job 全綠（✓ run `34042000019` head `88f2855`：checksums 6s、frontend 44s、go 4m36s、wails-build 3m10s）；implementation main run 全綠（✓ run `34043002480` main `ee30055`）；**final B2a HEAD（含 closure commit）的 main run 全綠**（待本 rev push 後）；各 run ID、SHA、耗時、runner image、工具版本、cache hit 記錄。
...
## 尚未完成
- **rev6 現況**：Task 1–2 完成，Task 3 的合併與 implementation main run 完成；只剩 closure commit push（另案授權）→ final main run 全綠 → rev7 回填 run id 並關票 → 另案申請刪除遠端 `b2a/ci-workflows`。B2b 另開 plan（承接事項見下）。
