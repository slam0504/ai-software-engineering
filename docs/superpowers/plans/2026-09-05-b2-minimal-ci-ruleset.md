...
> 版本：rev7（2026-09-07，**B2a 關票**：closure commit push `ee30055..19422bc` 觸發 final main run `34044380927`（push，headSha `19422bc`，attempt 1）四 job 全綠、artifact 與 check-runs 重驗通過；Gate A 全部成立；本 rev7 與 backlog rev26 為有限次回填（owner 裁定），其推送觸發的 CI 只保存於 GitHub run 紀錄與結案回報，不再為其新增 commit；遠端 `b2a/ci-workflows` 另案申請刪除；前版：rev6（2026-09-07，closure：PR run `34042000019`（head `88f2855`）四 job 全綠並完成 Gate A 重驗；owner 授權 PR #1 rebase and merge → main `ee300553944a6c71d3ea6a0980e5e7849772973b`（11 個線性 commit、tree 與 `88f2855` 相同）；implementation main run `34043002480`（push，headSha `ee30055`）四 job 全綠、artifact 重驗通過、check-runs 以 main SHA 抄錄為權威（四 context、app `github-actions`／15368）；本 rev6＋backlog rev25 為 closure commit，push 後觸發 final main run（另案授權）；前版：rev5（2026-09-07，B2c 系列結案後 Gate A 重啟：B2c-7 CI 驗證通過、register v7 #7 resolved（main `0e17e29`）；本機 `bd6a102`（遠端 `ffcd161`＋一個 plan-only 停止紀錄，共九個 commit）rebase 到 `main` `0e17e29`，`range-diff` 九個 commit 皆 `=`，`ci.yml`／README／SHA256SUMS 與 `ffcd161` 位元組相同、`ci.yml` 與 revert 後狀態一致；本 rev5 與 backlog rev24 作為兩個新 commit 加在其後；Gate A 改為待重新驗證，舊 run 只作歷史；force-push（`--force-with-lease` 鎖 `ffcd161`）、合併、implementation main run、closure 與 final main run 逐次另案授權；前版：rev4（2026-09-05，rev3 短複審 CHANGES_REQUIRED 後修訂：P1 Step 2b control 的 `rc=97` 改在寫 `.rc` 之前設定（原順序會讓 job exit 97 但 artifact `.rc` 為 0，owner 已重現）；P2 revert 檢查改 `git diff --exit-code <control>^ <revert> -- .github/workflows/ci.yml`；P2 `checksums` 的 diff-check step 改由 `env:` 傳入 Actions 值、shell 只讀環境變數，使同一段可在本機以 shell control 執行；前版：rev3（rev2 短複審 CHANGES_REQUIRED 後修訂：P1 三段測試 wrapper 改為 `set +e` → pipeline → 立即存 `PIPESTATUS[0]` → `set -e` → 寫 `.rc` → `exit`（GitHub `shell: bash` 實為 `bash -eo pipefail`，原寫法失敗時寫不出 `.rc`，owner 已重現）；P1 新增 Task 1 的暫時性 failure-path control；P2 final B2a HEAD 須再跑全綠並分開記錄兩次 main run、check-runs 抄 `app.id` 並以 main SHA 為權威、Gate A 事件覆蓋語意修正；前版：rev2（第一輪 owner CHANGES_REQUIRED 後修訂：D1–D6 裁定回寫（拆 B2a／B2b、`macos-15-intel`＋Go 1.26.5、SHA256SUMS 只留 m0／m1／m1.5、`ci-merge-policy.md`、0.6／0.7 pt、n=5 clean PR run attempts）；五項 P1——artifact 建置鏈閉合、README 可重現順序納入 scope、失敗時仍保存 artifact 且不假綠、checksums／`git diff --check` 可逐字執行、B2b closure PR 與外部寫入授權路徑；兩項 P2——required check context 以實際 run 讀取、量測樣本來源；前版：rev1）
> 狀態：**B2a 已關票（2026-09-07，rev7）——PR run、implementation main run、final main run 三次四 job 全綠；Gate A 全部成立；main `19422bc` 含 CI workflow；B2b（ruleset／enforcement／`ci-merge-policy.md`／D6 量測）另開 plan，required contexts 權威為 `go`／`wails-build`／`frontend`／`checksums`（app `github-actions`／15368）**。（rev6 狀態，歷史：closure（2026-09-07，rev6）——PR run 與 implementation main run 皆全綠、Gate A 除「final main run」外全部成立；PR #1 已合併（main `ee30055`）；本 closure commit push 後的 final main run 全綠即 B2a 關票，之後另案申請刪除遠端 `b2a/ci-workflows`（`88f2855`）**。（rev5 狀態，歷史：Gate A 待重新驗證（2026-09-07，rev5）——blocked 已解除（B2c-7 CI 驗證通過、register v7 #7 resolved）；分支已 rebase 到 `main` `0e17e29`（B2c-5／B2c-6 修法在 base 內）；等 owner 授權 force-push 更新 PR #1 後，以新 PR run 重新走 Gate A（四 job 全綠後重驗 artifact、rc、checksum、`.app` 權限、check contexts），再逐次另案授權合併、implementation main run、closure 文件與 final main run；舊綠燈不沿用，failure-path control 保留為歷史證據、workflow 內容一致時不重新注入；PR #1 開啟未合併、GitHub 設定面零變更**。（歷史，2026-09-05 停止紀錄：run `33953144191` attempt 1／2 於 `internal/claude/TestOrphanDoesNotHangNormalExit` 連續紅燈，依 owner 規則停止，#7 登記 register v3 候選，由 B2c 系列承接診斷——見 Task 1 證據段與 diagnosis-record v3。）B2b 另開 plan
...
- [x] （2026-09-07，owner 授權，鎖定 PR head `88f2855`／main base `0e17e29`／11 commit）`gh pr merge 1 --rebase` → PR #1 MERGED 15:39:31Z，main `ee300553944a6c71d3ea6a0980e5e7849772973b`；`git log 0e17e29..origin/main` 11 個 commit、`--merges` 0；`origin/main^{tree}`＝`88f2855^{tree}`（`d91e90dc…`）；遠端分支依裁定保留到 closure 與 final main run 之後。owner 授權後合併 PR（線性；GitHub 端若只有 merge／squash／rebase，選 **rebase and merge** 並記錄）；owner 授權後刪遠端分支。
- [x] （run **`34043002480`**，push，headSha `ee30055`，attempt 1，2026-09-06 15:39:33Z 起，四 job success：checksums ubuntu-24.04 15:39:36–15:39:44（8s）、frontend ubuntu-24.04 15:39:36–15:40:29（53s，vitest 40 檔 397 passed、`vitest.rc`／`frontend-build.rc` 0）、go macos-15（`macos-15-intel`，Go 1.26.5）15:40:32–15:47:42（7m10s，`go-test.rc` 0、1358 pass／0 fail／2 skip、無 race／panic）、wails-build macos-15 15:40:40–15:46:15（5m35s，`.app` tar 解開後 `Contents/MacOS/sdlc-workbench` 可執行 Mach-O x86_64）；`frontend-dist/index.html` 存在；**check-runs（權威，main SHA `ee30055`）**：`go`／`wails-build`／`frontend`／`checksums` 四個 context，app slug `github-actions`、**app id 15368**，皆 success；八條牆鐘測試 elapsed（補充）：#1 0.07s、#2 0.51s、#3 0.06s、#4 0.04s、#5 0.07s、#6 0.35s、#7 0.05s、兩層 fork 0.01s；證據 `/tmp/b2a-mainrun.*`）**implementation main run**：合併後 main 上 `push` 觸發跑一次全綠；抄錄 run ID、四個 job 耗時與 check-runs（含 `app.id`）——B2b 建 ruleset 的權威 context 清單。
- [x] （run **`34044380927`**，push，headSha `19422bca4225c4e0765b4a314d29fc09eae1e581`，attempt 1，2026-09-06 16:06:17Z 起，四 job success：checksums ubuntu-24.04 16:06:20–16:06:26（6s）、frontend ubuntu-24.04 16:06:20–16:07:17（57s，vitest 40 檔 397 passed，`vitest.rc`／`frontend-build.rc` 0）、go macos-15 16:07:20–16:13:46（6m26s，`go-test.rc` 0、1358 pass／0 fail／2 skip、無 race／panic、`TestOrphanDoesNotHangNormalExit` 0.04s、兩層 fork 0.01s）、wails-build macos-15 16:07:21–16:10:57（3m36s，`.app` 主程式可執行 Mach-O x86_64）；`frontend-dist/index.html` 存在；check-runs（`19422bc`）四 context app `github-actions`／15368 皆 success；證據 `/tmp/b2a-finalrun.*`。Gate A 綁定 final HEAD `19422bc`。）**closure-commit main run**：plan／backlog closure commit push 後，final B2a HEAD 再跑四個 job 全綠，與 implementation main run 分開記錄；Gate A 綁 final HEAD。
...
- **revert**：`d7cd55c`（`git revert 776c196`），`git diff --exit-code 776c196^ d7cd55c -- .github/workflows/ci.yml` **rc 0**；YAML 可解析。owner 授權後推送 `776c196..ffcd161`（含證據回填 `ffcd161`）。
- **正式綠燈候選 run `33953144191`（head `ffcd161`，workflow bytes 與 run 1 相同）——Gate A 停止**：
  - **attempt 1**（07:39Z）：`checksums`／`frontend`／`wails-build` 成功（`vitest.rc`、`frontend-build.rc` 0；`.app` 可執行）；**`go` 失敗**（`go-test.rc`=1）。唯一 FAIL：`internal/claude` **`TestOrphanDoesNotHangNormalExit`** 5.01s，逐字 `session_test.go:207: drain/Wait hung on orphan-held pipes`（測試自身 `select`＋`time.After(5 * time.Second)` guard）；同套件其餘 34 條 0–0.06s；root 144.9s、`internal/proc` 38.2s 正常；無 `-timeout` panic、無 DATA RACE。artifact `go-test-json`（id 9965561717）本機保存，hash 見 `/tmp/b2a-attempt1-artifacts-sha256.txt`（`go-test.json` `46d5466e…`）。**此測試不在 register 八條內**（§7／register／backlog 皆無）。
  - **本機反證（同日，8 核 x86_64）**：focused `-race -count=30` 全過 2.02s；三份 `./internal/...` `-race` 併發負載下 focused `-count=20` 最慢 0.02s；B1a-4 各層 0.01–0.03s；run 1（`33952217785`）0.04s。
  - **attempt 2**（owner 一次性診斷例外，`gh run rerun 33953144191 --failed`，08:00Z，只重跑 `go`）：**同一測試、同一訊息再紅**（5.01s，`go-test.rc`=1；artifact id 9965826248，`go-test.json` `1c0ff62e…`）；同名 artifact 第二次上傳成功（無 409）；其餘套件全過。→ **現行 HEAD `ffcd161` 上 CI 2/2 重現**；`109b407`（`internal/claude` 與 workflow 相關 bytes 相同）的 run 1 一次綠燈另列為跨 SHA 對照，不併入主要狀態。
  - **runner 對照**：run 1 綠與 run 3 兩次紅皆為 `macos-15-intel`、Image macos-15 **Version 20260824.0482.1**（同一 image，不同 VM：runner 1000000827／834／835）；Go 1.26.5、Node 26.8.1 相同。排除 image 漂移。
  - **機制事實（唯讀）**：`proc.go:152` stdout／stderr 皆為 `os.Pipe` write end（無 copy goroutine），`cmd.Wait()` 於 leader 退出即返回；`proc.go:191` 隨即 `SignalGroup(SIGKILL)` 清孫程序，不經 grace；fixture `testdata/fake-claude.sh:16` 孫程序為 `bash -c 'trap "" TERM; sleep 30' &`（持有 stdout 30 秒）。故 CI 上 5 秒內無 EOF ＝ 該次 group KILL 未及時使孫程序放開 stdout（或 `cmd.Wait` 延遲、或 EOF 傳遞延遲）——三者未區分，**根因未確認**。
  - **裁定執行**：依 owner 規則「attempt 2 再紅則 Gate A 停止」，**B2a Gate A 停止於此**；不再重跑。此測試登記為 register Go 表 **#7**（狀態：candidate；**現行 HEAD `ffcd161` 上 CI 2/2 重現**；`109b407`（相同相關 bytes）的一次綠燈另列為跨 SHA 對照；本機 bounded stress 未重現；根因未確認），續票 **B2c**（診斷／preflight，0.4 pt）承接；不得以放寬 5 秒或加 retry 作修法。PR #1 維持開啟、未合併；GitHub 設定面零變更。

## Gate A（B2a 完成條件）
- [x] PR 內四個 job 全綠（✓ run `34042000019` head `88f2855`：checksums 6s、frontend 44s、go 4m36s、wails-build 3m10s）；implementation main run 全綠（✓ run `34043002480` main `ee30055`：8s／53s／7m10s／5m35s）；**final B2a HEAD（含 closure commit）的 main run 全綠**（✓ run `34044380927` main `19422bc`：6s／57s／6m26s／3m36s）；三次 runner image 皆 ubuntu-24.04（checksums／frontend）與 macos-15（go／wails-build），Go 1.26.5、Wails CLI v2.13.0；npm cache 狀態以各 run log 為準；各 run ID、SHA、耗時、runner image、工具版本、cache hit 記錄。
- [x] （PR run 與 main run 皆重驗：`index.html` 存在、`sdlc-workbench.app.tgz` 解開後主程式 `-rwxr-xr-x` Mach-O x86_64、`go-test.rc`／`vitest.rc`／`frontend-build.rc` 皆 0；failure path 沿用 Step 2b control 歷史證據 run `33952896114`，workflow 內容與 revert 後一致故不重新注入）artifact 鏈：`frontend-dist` → `go`／`wails-build` 皆 `needs: frontend` 且下載後驗 `index.html`；`wails-app-tar` 下載解開後主程式可執行；`go-test-json`／`vitest-output` 含 `.rc`＝0；failure path 已由 Step 2b control 實證：測試 job 為紅、`.rc` 為非零（97 或實際值）且 artifact 可下載，control commit 已 revert（diff 為空）。
- [x] （`pull_request` 路徑：run `34042000019`；`push` 正常路徑：run `34043002480`；兩次 `checksums` job 皆 success，log 逐檔 `: OK`。**`workflow_dispatch` 與 `push` 零 SHA 路徑以本機 shell control 實測（2026-09-07，未觸發遠端 dispatch）**：以 YAML parser 從 `ci.yml` 逐字抽出 `checksums` job step 3 的 `run:`（SHA-256 前 16 `5d5304002724633a`），在 closure commit `706f30f` 上以 `bash -eo pipefail` 執行——`EVENT_NAME=workflow_dispatch SHA=706f30f…` → `git diff-tree -r --check --root` rc 0；`EVENT_NAME=push BEFORE_SHA=0000…0 SHA=706f30f…` → 同路徑 rc 0；`EVENT_NAME=push BEFORE_SHA=ee30055… SHA=706f30f…` → `git diff --check` rc 0；`EVENT_NAME=schedule` → `unsupported event` rc 1（fail-loud 分支有效）；step 1／2 的 `shasum -a 256 -c` 於 `schemas/codex`（275 筆 OK）與 `docs/architecture`（3 筆 OK）rc 0；證據 `/tmp/b2a-checksums-ctl.*`）`checksums`：兩份 SHA256SUMS 於各自目錄 `-c` 全 OK；`git diff --check` 的 `pull_request` 分支與 `push` 分支各實跑一次並綠；`workflow_dispatch` 分支**不混入**——以本機 shell control 驗證：把 step 的 `run:` 區塊逐字抽出，以 `EVENT_NAME=workflow_dispatch SHA=<sha>`（及 `EVENT_NAME=push BEFORE_SHA=000…0 SHA=<sha>` 零 SHA 路徑）在本 repo 執行並記錄 rc；或另經 owner 授權後手動 dispatch 一次實跑並記錄。
- [x] （已於 `109b407`／`0db8921` 落地並隨 rebase 進 main `ee30055`；內容與 `ffcd161` 位元組相同）README §測試順序與 CI job 逐項對應；:380 一句同步；SHA256SUMS 三筆。
- [x] （main SHA `ee30055`：`go`／`wails-build`／`frontend`／`checksums`，app `github-actions`／15368）check-runs 的四個 context 名稱、app slug 與 app id 已以 implementation main SHA 為權威抄錄（PR head 版本為預檢）。
- [x] （`git diff --name-only 0e17e29..ee30055`：`ci.yml`、`SHA256SUMS`、README、本 plan、backlog 五檔；零 `.go`／`.ts`／`.vue`／config；GitHub 設定面零變更）repo 變更只含 `.github/workflows/ci.yml`、`docs/architecture/SHA256SUMS`、`README.md`、本 plan、backlog；零 `.go`／`.ts`／`.vue`／config；GitHub 設定面零變更。

---

## B2b 承接事項（另開 plan 時的已裁定輸入，本 plan 不執行）

- **範圍**：(2) main ruleset（required contexts 以 B2a 抄錄的 check-runs 為準；require PR、線性歷史、禁 direct push；**不設常駐 bypass actor**）；(5a) probe PR 故意讓 `checksums` 紅→不可合併；(5a′) probe 改 job 名使 required context 缺席→永遠 pending（＝(5d) fail loud 實證）；(5b) 修正後全綠→`mergeStateStatus: CLEAN`；(5c)(5e) `docs/architecture/ci-merge-policy.md`＋automation plan §12 指向；(6) n=5 clean PR run attempts 量測 #2／#3／#6／F1／F2，回寫 register **v4**（v3 由 B2a 的 #7 候選登記占用）；backlog rev17 關票。
...
## 尚未完成
- **rev7：無**——B2a 關票。剩餘事項不屬本票：遠端 `b2a/ci-workflows`（`88f2855`）刪除另案申請；B2b 另開 plan（承接事項見下）；本 rev7／backlog rev26 推送觸發的 CI 只保存於 GitHub run 紀錄與結案回報。
- （歷史）**rev6 現況**：Task 1–2 完成，Task 3 的合併與 implementation main run 完成；只剩 closure commit push（另案授權）→ final main run 全綠 → rev7 回填 run id 並關票 → 另案申請刪除遠端 `b2a/ci-workflows`。B2b 另開 plan（承接事項見下）。
- （歷史）**rev5 現況**：rebase 完成、Gate A 待重新驗證；force-push 待授權；Task 3 三步待逐次授權；backlog rev24 隨本分支進 PR #1，關票時另出 rev。
...
## 修訂記錄
- rev7（2026-09-07）：B2a 關票——final main run `34044380927`（headSha `19422bc`）四 job 全綠、artifact／check-runs 重驗；Task 3 第三步與 Gate A 第一項勾選；狀態改已關票；有限次回填（本 rev 與 backlog rev26 一次推送，其 CI 不再回填）。
- rev6（2026-09-07）：closure——PR run `34042000019` 全綠與 Gate A 重驗、PR #1 rebase and merge（main `ee30055`，11 線性 commit、tree 相同）、implementation main run `34043002480` 全綠與 artifact 重驗、check-runs 權威抄錄（四 context、app id 15368）；Gate A 除 final main run 外勾選；Task 3 前兩步勾選、第三步待本 rev push。
