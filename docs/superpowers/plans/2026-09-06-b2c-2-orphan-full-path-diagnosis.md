...
- **證據有效性兩層級**：(1) **單輪**——`evidenceValid`（layer 3 record）：Start 失敗、stream error、ps 取樣錯誤、kill0／ps sampler 逾界皆經 `invalidateRecord` 設 false 並寫 `rec.Errors`，Task 4 只以 `evidenceValid=true` 的樣本入分母；(2) **整批**——artifact 寫檔、record／summary 編碼、JSONL 開關失敗只計入 `invalidEvidence`，由 summary `invalidEvidenceCount>0` 判定整次 run 無效。兩層級的錯誤都令測試 rc 非零。
...
## 尚未完成
- Task 1 通過；Task 2 完成（`932dbc3`＋修正 `086e12a`／`0b3454f`／`84b31cd`），owner 檢查點待過；Task 3（workflow＋一次推送）未開始；三點 scope 現為六檔。Task 4 未執行。
