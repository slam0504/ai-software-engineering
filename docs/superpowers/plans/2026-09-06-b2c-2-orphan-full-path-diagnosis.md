...
## 尚未完成
- Task 1 通過；Task 2 完成（`932dbc3`），owner 檢查點待過；Task 3（workflow＋一次推送）未開始；三點 scope 現為六檔。Task 3／4 未執行。三點 scope 目前五檔（尚缺 `internal/claude/orphan_diag_test.go`）。
