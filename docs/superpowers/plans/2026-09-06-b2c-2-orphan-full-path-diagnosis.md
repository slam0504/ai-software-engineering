...
## 尚未完成
- Task 1 通過；Task 2（layer 3）進行中；Task 3／4 未執行。三點 scope 目前五檔（尚缺 `internal/claude/orphan_diag_test.go`）。
