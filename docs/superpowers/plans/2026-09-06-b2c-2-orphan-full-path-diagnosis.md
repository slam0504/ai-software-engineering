...
- **證據有效性兩層級**：(1) **單輪**——`evidenceValid`（layer 3 record）：Start 失敗、stream error、ps 取樣錯誤、kill0／ps sampler 逾界皆經 `invalidateRecord` 設 false 並寫 `rec.Errors`，Task 4 只以 `evidenceValid=true` 的樣本入分母；(2) **整批**——`iter-records.jsonl` 無法建立時直接 `t.Fatalf`（fail loud，不產生 summary）；成功建立後的 encode／close、artifact 寫檔、record／summary 編碼失敗計入 `invalidEvidence`，由 summary `invalidEvidenceCount>0` 判定整次 run 無效。兩層級的錯誤都令測試 rc 非零。
...
- [x] `diag-orphan.yml` 改為三個 step（`d7ca50d`；與下方逐字契約一致，另保留 env／ps-job-start／residue snapshot 步驟，env 增列 fake-claude 可執行與 ambient FAKE_* 計數——**rev14 修正 `1e732ee`**：`test -x` 改為獨立命令、計數改 awk；`bash -eo pipefail` 實測（`/tmp/b2c2-envstep.sh` 由 YAML 抽出）：A 零個 ambient FAKE_* → rc 0、`ambient FAKE_* count: 0`、env.txt 9 行；B `FAKE_DIE=1 FAKE_HANG=1` → rc 0、count 2；C fake-claude 路徑不存在 → rc 1、輸出停在 diag fixture 行；D diag fixture 路徑不存在 → rc 1、輸出停在 go version 行；ruby YAML 解析：steps=8（checkout／setup-go／env／2b／3／parity／residue／upload）、三個 DIAG_OUT 互不重疊、`timeout-minutes: 90`、`grep -c DIAG_FAKE_` = 0）（皆 `if: always()`，各自輸出目錄不重疊，各自 `go test -timeout`，wrapper 契約保留 rc，upload 路徑逐字如下；移除 layer 1）：
...
- [x] 本機：`go vet -tags diag_orphan ./internal/proc ./internal/claude` OK、無 tag 隔離（`go test -list Diag ./internal/...` 為空）、`gofmt -l` 空、`go build ./...` OK、YAML 解析 OK；scope：`git diff --name-only origin/main...HEAD` 六個檔。
...
## 尚未完成
- Task 1／2 通過；Task 3 Step 1 完成（`d7ca50d`＋`1e732ee`），待 owner 授權一次推送；Task 3 推送後 Gate A（四 job artifact）與 Task 4 解析未執行；三點 scope 現為六檔。Task 4 未執行。

## 修訂記錄
- rev14（2026-09-06）：推送前複審 2 P1——env step fail-loud 與 awk 計數修正 `1e732ee`，四個 shell 控制證據。
- rev13（2026-09-06）：Task 2 APPROVED；Task 3 Step 1 workflow `d7ca50d` 與本機檢查；P2 JSONL 建立失敗措辭修正；待推送授權。
- rev12（2026-09-06）：Task 2 第三輪修正 `84b31cd`（`invalidateRecord` 統一單輪失效；證據有效性兩層級寫入 Global Constraints）。控制（`/tmp/b2c2-t2r4.*`）：bad-line `evidenceValid=false`＋`errors` 含 stream error；ps-timeout `evidenceValid=false`＋4 筆 errors；`DIAG_ITER=3` 全 true、無 errors；once-only／aggregator PASS。
