...
> 版本：rev1（2026-09-07，design gate 第一輪；待 owner 裁定 D1–D6）
...
> 授權邊界：本票修改 production（`internal/proc`、`internal/ports`、`internal/claude`、`internal/codex`、`internal/recorder`、`internal/assist`、`app.go` 揭露點）與既有測試；**不改 fixture、不放寬 5 秒 guard、不加 retry 於被測路徑、不改 `Terminate()` 介面與升級路徑、不改 `Done()` 以外的既有語意**。合併 main 需 owner 授權（fast-forward push）。B2a 維持 blocked。
...
- [ ] **Step 3 白箱測試**（新檔 `internal/proc/proc_cleanup_test.go`）：以 `bashProc` 起一個**無 orphan、`read` 等 stdin** 的 leader（與既有 `no_orphan` 案例同形）作為真實 Proc 載體，注入三個 seam 後送 stdin 讓 leader 退出，seam 以 channel 記錄每次呼叫（避免 `-race`）；六案：
  ...
  4. (iv) `cleanupSignal`→`nil`、`groupProbe` 恆 `nil`、rekill 恆 `nil`：rekill 10、之後 1 s 確認（`cleanupAfter` 第 11 次 duration≈1 s）仍 `nil` → `CleanupIncomplete=true`；`Wait()` 於注入時間表下 <1 s 返回；`Done()` 已關；並行的 stdout reader 收到錯誤且 `errors.Is(err, ErrCleanupIncomplete)`；stderr tail 不含我方敘述。**對照案例**：同序列但呼叫端在強制關閉前自行 `p.Stdout.Close()` → reader 得到的錯誤**不**滿足 `errors.Is(ErrCleanupIncomplete)`。
  ...
  另：`stdoutReader` 單測（`Read` 映射三態、`Close` 冪等）；`classifyKillErr` 單測（`syscall.Errno` 與 wrap 皆可辨識）。
- [ ] **Step 4 既有測試調整**（D6）：改名與逐種類計數；escalation 測試容許 rekill；`killAndReap` 不變。
- [ ] **Step 5 本機控制**：`go vet ./internal/proc`、`gofmt -l`、`go test -race ./internal/proc -count=3`；mutation（各改一處、跑對應案例、還原）：移除 rekill → (ii) 紅；`EPERM` 當終止 → (iii) 紅（探測次數 <3 或 `CleanupIncomplete`）；移除強制關閉 → (iv) 紅（`Wait()` 逾時，以測試內 3 s guard 判）；第一次 KILL 非 `nil` 就不排程 → (v) 紅；rekill 失敗也發事件 → 新增案例 (ii') `rekill→EPERM` 時 rekill 事件 0 紅。逐字記錄。
...
- [ ] 全套 `go test -race ./... -count=1`（本機 8 核）PASS；`TestOrphanDoesNotHangNormalExit` focused `-count=30` PASS（本機不重現形狀，只作回歸）；既有 orphan 案例（`TestNormalExitReapsOrphanAndCachesExit`、`TestCtxCancelKillsWholeGroup`、`TestTerminateEscalatesToGroupKill`）`-count=20` PASS。
...
- [ ] Task 1 六案＋對照＋helper 單測 PASS；五條 mutation 各自紅並逐字記錄；讀碼審查六項通過。
- [ ] Task 2 四個揭露點各有測試或讀碼確認；`FinalizeCause`／`Exit.Err` 語意未變。
- [ ] 全套 `-race` PASS；scope 只含範圍內檔案；`Terminate`／escalation／fixture／guard 零變更。
