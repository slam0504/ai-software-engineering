...
> 版本：rev2（2026-09-07，決策 gate 第一輪 CHANGES_REQUIRED 後修訂：兩項 P1——EPERM 不得視為終止（`P_REF_NEW` 亦會造成 EPERM）、預算耗盡須有強制解除 pipe 等待的 fail-loud 路徑並把 `CleanupIncomplete` 傳到 `ports.Exit`／codex meta／assist；D1–D7 裁定回寫（固定絕對偏移 1–512 ms＋1 s 確認、`Exit.Err` 不混入、oracle 只以 ESRCH 為消失、rekill 事件與既有測試改名、估點 0.6／0.3／0.2、`b2c7/verify`、#7 resolved 完整條件）；三處措辭限縮；同分支附 main diagnosis-record／register 的 EPERM 勘誤；前版：rev1）
> 狀態：**待 owner 複核 rev2**。本票不修改任何 code；產出為裁定記錄、implementation 票面與估點、register #7 處置路徑。本文件在 owner 核准後即為 B2c-4 的裁定記錄（backlog rev20 落地時引用）；EPERM 勘誤隨本票 docs-only 推送落地，不等 B2c-7。
...
- **時間表（D2，固定、不開放 `Config` 覆寫）**：以 `SignalGroup(SIGKILL)` 成功返回的 monotonic 時刻為 base，於絕對偏移 **1／2／4／8／16／32／64／128／256／512 ms** 各做一次 `kill(-pgid, 0)` 探測，最後於 **1 s** 做一次**不送訊號**的確認。每個探測點：回 **`ESRCH`** → 群組確定不存在，結束（成功）；回 **`nil`** → 仍有可送達成員 → 立即 `SignalGroup(SIGKILL)`（記 rekill 事件）並繼續下一個偏移；回 **`EPERM`** → 「目前無可送達成員／狀態未知」（可能是 zombie、exit transition，或**建立中的 `P_REF_NEW` 成員**）→ 本輪**不送 KILL**，但**繼續**後續排程；其他 errno → 記錄並繼續。1 s 確認點：`ESRCH` → 成功；`nil` 或 `EPERM` → **預算耗盡**，進入 fail-loud 路徑。時間表包含一次晚於 B2c-3 +300 ms 證據點的重送（512 ms），且最後一次重送後仍有確認點（1 s）。
- **fail-loud 路徑（D3 擴充版 (a)）**：預算耗盡時 (1) 設 `Exit.CleanupIncomplete=true`；(2) 關閉 proc 持有的 stderr read end（`errR`）並讓 stderr reader 結束，`wg.Wait()` 不再依賴 EOF；(3) 關閉 proc 持有的 stdout read end（`p.Stdout`），呼叫端的 reader 會收到讀取錯誤——為了讓呼叫端能分辨「supervisor 放棄清理」與其他 I/O 錯誤，`p.Stdout` 以薄包裝提供，強制關閉後的 `Read` 回傳具名錯誤 **`proc.ErrCleanupIncomplete`**（實作細節屬 B2c-5，見 D8）；(4) 快取 `Exit`、`close(doneCh)`。之後逃脫者若仍存活，屬揭露後的已知殘留，不再由本 Proc 負責。
...
- **不改變**：`Done()` 仍以「`Exit` 已快取」為語意（EOF 或強制解除其一）；`Wait()`／`Terminate()`／`PGID()` 介面不變；`sigEventSupervisorCleanupKill` 仍只在第一次 group KILL 成功時發一次；重送另發 **`sigEventSupervisorCleanupRekill`**（每次一次）；預算耗盡另發 **`sigEventSupervisorCleanupGaveUp`**（一次），供白箱測試逐種類斷言。
- **seam（D5）**：偏移時間表經**獨立、nil-safe 的 seam**（沿 B1a-1 `seamAfter` 慣例，例如 `cleanupAfter afterFunc`，nil 時退回真實時鐘），白箱測試可把整個時間表壓縮；`kill(-pgid, 0)` 探測經可注入的 `groupProbe func(pgid int) error`（nil 時退回 `syscall.Kill`），白箱測試可模擬 `nil`／`EPERM`／`ESRCH` 序列。
...
| 票 | 範圍 | 驗收條件 | 估點 |
|---|---|---|---|
| **B2c-5** proc supervisor 有界清理（O1 修正版） | `internal/proc/proc.go` supervisor：固定絕對偏移 1–512 ms 探測＋`nil` 即重送、`ESRCH` 終止、`EPERM` 非終止、1 s 不送訊號確認；預算耗盡 fail-loud：`Exit.CleanupIncomplete`、關閉 stderr／stdout read end、`p.Stdout` 薄包裝回 `ErrCleanupIncomplete`、快取 Exit 與 `close(doneCh)`；新增 seam：`cleanupAfter`（nil-safe）、`groupProbe`（nil-safe）、事件 `sigEventSupervisorCleanupRekill`／`sigEventSupervisorCleanupGaveUp`；`ports.Exit.CleanupIncomplete`、claude `Session.Wait` 對應、codex `recorder.Meta.cleanup_incomplete`（`FinalizeWith` 寫入）、assist one-shot 回傳 wrap 錯誤；既有事件測試改名為「第一次 cleanup signal 成功」語意並逐事件種類斷言精確次數；白箱測試以 `groupProbe`＋`cleanupAfter` 模擬序列：(i) 立即 `ESRCH`（0 rekill、0 gave-up）；(ii) `nil`×N 後 `ESRCH`（N 次 rekill）；(iii) `EPERM`×k 後 `ESRCH`（0 rekill、繼續排程、成功）；(iv) 到 1 s 仍 `nil`／`EPERM`（gave-up 一次、`CleanupIncomplete=true`、`Wait()`／`Done()` 有界返回、stdout reader 收到 `ErrCleanupIncomplete`）；真實 fixture 測試沿用 `proc_test.go` 既有 orphan 案例並新增**兩層 fork** 案例（`[ -n … ] && bash -c '…' &` 結構）作 CI 證據（本機不保證重現）；`Terminate` 升級路徑不改（假設 A6 由 B2c-7 檢查） | (1) 既有 proc／claude／codex／assist 測試無 tag 全 PASS（`-race`）；(2) 白箱四態各一案，mutation：移除重送 → (ii) 紅；把 `EPERM` 當終止 → (iii) 紅（斷言成功前仍有後續探測）；移除強制關閉 → (iv) 紅（`Wait()` 逾時）；(3) `sigEventSupervisorCleanupKill` 恰一次、rekill 恰 N 次、gave-up 恰 0／1 次，逐種類斷言；(4) `Done()` 仍以「Exit 已快取」為語意、`Terminate` 介面不變、`Exit.Err` 語意不變；(5) codex／assist／claude 的揭露各有一條測試 | **0.6 pt**（5.0–7.0 hr，中位 6.0；owner D6） |
...
- **待複核（rev2 新增，主 agent 提案）**：
  - **D8**：強制關閉 stdout read end 時，呼叫端讀到的錯誤以 `p.Stdout` 薄包裝回具名錯誤 `proc.ErrCleanupIncomplete`（而非 `os.ErrClosed`），使 claude `KindStreamError`／assist 回傳錯誤可辨識來源；`Done()` 語意改述為「`Exit` 已快取（EOF 或強制解除其一）」。是否核准此細節，或改由 B2c-5 plan gate 決定。
  - **D9**：預算耗盡另發 `sigEventSupervisorCleanupGaveUp` 事件（一次），供白箱測試逐種類斷言；是否核准。
  - **D10**：EPERM 勘誤同分支落地：diagnosis-record §7.2／§7.3 (f)／§7.5 (4) 與 register #7 row 的 EPERM 句子補上 `P_REF_NEW`，record 修訂記錄加 v2 勘誤條目、register 修訂記錄加 v6 勘誤條目（版本號不變）。是否核准與本票同一次 docs-only 推送。
