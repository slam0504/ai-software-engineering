...
describe('evidence store', () => {
  ...
  it('started → finished(passed) transitions running→passed and records evidence_id', () => {
    ...
    e.applyEvidenceEvent(env({ phase: 'finished', plan_id: 'P1', task_id: 'T1', kind: 'expected_red', evidence_id: 'ev-1', result: 'passed' }))
  ...
  it('finished with an error payload lands as status=error and keeps the raw message', () => {
    ...
    e.applyEvidenceEvent(env({
      phase: 'finished', plan_id: 'P1', task_id: 'T1', kind: 'negative_control',
  ...
  it('a finished event with no matching run is a no-op elsewhere but still lands on its own (plan,task,kind) cell', () => {
    const e = useEvidence()
    e.applyEvidenceEvent(env({ phase: 'finished', plan_id: 'P1', task_id: 'T1', kind: 'expected_red', evidence_id: 'ev-3', result: 'passed' }))
    // finished 不再需要事先有 started（review fix：直接用 payload 的三個識別
    // 欄位定位，不靠 pendingKey FIFO），所以會直接落地在自己的格子。
    expect(e.runOf('P1', 'T1', 'expected_red')).toMatchObject({ status: 'passed', evidenceId: 'ev-3' })
    // 其他格子不受影響。
    expect(e.runOf('P1', 'T1', 'negative_control')).toBeUndefined()
  })

  it('缺 plan_id/task_id/kind 任一識別欄位的事件不落地（無法定位格子就不猜）', () => {
    const e = useEvidence()
    e.applyEvidenceEvent(env({ phase: 'started', plan_id: 'P1', task_id: 'T1' })) // 缺 kind
  ...
  it('兩筆 run 交錯 started/finished 各自落在正確格子，不因交錯順序錯位', () => {
    ...
    e.applyEvidenceEvent(env({ phase: 'finished', plan_id: 'P1', task_id: 'T1', kind: 'expected_red', evidence_id: 'ev-red', result: 'passed' }))

    expect(e.runOf('P1', 'T1', 'expected_red')).toMatchObject({ status: 'passed', evidenceId: 'ev-red' })
    // negative_control 必須仍在執行中，不能被 red 的 finished 誤標成 passed。
    expect(e.runOf('P1', 'T1', 'negative_control')).toMatchObject({ status: 'running' })
    expect(e.taskHasRunInFlight('P1', 'T1')).toBe(true)

    e.applyEvidenceEvent(env({ phase: 'finished', plan_id: 'P1', task_id: 'T1', kind: 'negative_control', evidence_id: 'ev-neg', result: 'failed' }))
    expect(e.runOf('P1', 'T1', 'negative_control')).toMatchObject({ status: 'failed', evidenceId: 'ev-neg' })
    expect(e.taskHasRunInFlight('P1', 'T1')).toBe(false)
  })
