...
describe('SessionList（Task 27，spec §4）', () => {
  ...
  // （at === focused === 0），如果實作把 `s.pin(at, wsid)` 誤換成
  // `s.pin(s.focused, wsid)`，那條測試不會變紅——凍結的危害（用 focused 會
  // 把卡片搬格並 releaseView 掉該格原本的 session）沒有守門。這裡補雙 pane
  // fixture：w2 先釘在 pane 1，w1 首載失敗清空 view 後把 focused 切到 1
  // （與 w1 實際釘選的 pane 0 不同）再點重試，斷言 binding 對 w1 第二次
  // 呼叫，且 pane 1 的 w2 沒被踢掉、pane 0 仍是 w1。
